...
static union fpregs_state fp_state;
...
static void print_fp_state(struct seq_file *m){
	if(!ts1){
		seq_printf(m, "task_struct with pid %d has not been fond.\n", pid); 
        }
	else{
       	    	fp_state  = ts1->thread.fpu.state;
...
static int __init kmod_init(void) {
    pr_info("fpu: module loading.\n");
	
    dir = debugfs_create_dir("fpu", NULL);
    pid_file = debugfs_create_file("pid_here", 0722, dir, NULL, &pid_ops);
